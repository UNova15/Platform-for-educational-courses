...
async function apiFetch(endpoint, options = {}) {
  ...
  if (response.status === 403) {
    throw new Error(
      "Доступ запрещён (403). Возможно, у вашей учётной записи нет нужных прав.",
    );
  }
...
export function updateCourse(courseId, courseData) {
  return apiFetch(`/teacher/courses/${courseId}`, {
    method: "PUT",
    body: courseData,
  });
}
...
export function createLesson(courseId, moduleId, lessonData) {
  return apiFetch(`/teacher/courses/${courseId}/modules/${moduleId}/lessons`, {
    method: "POST",
    body: lessonData,
  });
}

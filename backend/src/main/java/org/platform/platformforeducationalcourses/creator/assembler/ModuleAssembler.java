...
public class ModuleAssembler {
    ...
    public List<CourseModuleFindResponse> createCourseModuleFindResponse(List<Lesson> lessons, List<Test> tests, List<CourseModule> modules) {
        //группировка уроков по id модуля
        Map<Long, List<Lesson>> lessonByModule = lessons.stream()
                .collect(Collectors.groupingBy(Lesson::getModuleId));

        //группировка тестов по id модуля
        Map<Long, List<Test>> testByModule = tests.stream()
                .collect(Collectors.groupingBy(Test::getModuleId));
    ...
    public List<StudentModuleFindResponse> createStudentModuleFindResponse(List<CourseModule> modules, List<Lesson> lessons,
                                                                           List<LessonProgress> lessonProgresses, List<Test> tests,
                                                                           List<TestSubmission> testSubmissions) {
        List<StudentLessonFindResponse> findLessons = lessonAssembler.createStudentLessonFindResponse(lessons, lessonProgresses);
        List<StudentTestFindResponse> mappedTests = testAssembler.createStudentTestFindResponse(tests, testSubmissions);
        //группировка тестов по id модуля
        Map<Long, List<StudentTestFindResponse>> testsOrderByModuleId = mappedTests.stream()
                .collect(Collectors.groupingBy(StudentTestFindResponse::moduleId));
        //группировка уроков по id модуля
        Map<Long, List<StudentLessonFindResponse>> lessonOrderByModuleId = findLessons.stream()
                .collect(Collectors.groupingBy(StudentLessonFindResponse::moduleId));

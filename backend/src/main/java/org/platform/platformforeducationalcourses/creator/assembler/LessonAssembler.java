...
public class LessonAssembler {
    ...
    public List<StudentLessonFindResponse> createStudentLessonFindResponse(
            List<Lesson> lessons, List<LessonProgress> lessonProgresses) {
        Map<Long, LessonProgress> progressByLessonId =
                lessonProgresses.stream().collect(Collectors.toMap(LessonProgress::getLessonId, progress -> progress));

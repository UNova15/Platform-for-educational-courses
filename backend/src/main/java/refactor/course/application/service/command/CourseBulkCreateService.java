...
@Service
@AllArgsConstructor
public class CourseBulkCreateService implements CourseCreateUseCase {
    ...
    @Transactional
    public CourseCreateResult createCourseWithContent(CourseCreateCommand command) {
        Course course = Course.createNew(
                command.userId(),
                CourseTitle.of(command.title()),
                CourseDescription.of(command.description()),
                command.tag());

...
class UserTest {
    ...
    void createNew_ThrowsException_WhenLoginIsInvalid(String invalidLogin) {
        PasswordEncoder mockEncoder = mock(PasswordEncoder.class);
        assertThrows(IllegalArgumentException.class, () -> User.createNew(invalidLogin, "password123", DUMMY_ROLE, mockEncoder));
    }
    ...
    void createNew_ThrowsException_WhenPasswordIsInvalid(String invalidPassword) {
        PasswordEncoder mockEncoder = mock(PasswordEncoder.class);
        assertThrows(IllegalArgumentException.class, () -> User.createNew("login", invalidPassword, DUMMY_ROLE, mockEncoder));
    }
}
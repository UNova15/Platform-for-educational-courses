    ...
    public static RawPassword of(String password) {
        if (password == null || password.isBlank()) {
            throw new DomainValidationException(
                    AuthExceptionCode.INVALID_PASSWORD_FORMAT, "Empty data to create raw password");
        }

        if (password.length() < MIN_PASSWORD_LENGTH || password.length() > MAX_PASSWORD_LENGTH) {
            throw new DomainValidationException(
                    AuthExceptionCode.INVALID_PASSWORD_FORMAT,
                    "Incorrect password length: %d . Expected length: min: %d; max: %d"
                            .formatted(password.length(), MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH),
                    Map.of("length", password.length()));
        }
